	...
	function form( $instance ) {
		...
		<p>
            ...
            <br /><select id="<?php echo $this->get_field_id( 'id' ); ?>" name="<?php echo $this->get_field_name( 'id' ); ?>"><?php
				$lists = get_posts( 'post_status=publish&post_type='. W4PL_Config::LIST_POST_TYPE . '&posts_per_page=-1' );
				$id = (int) $id;
			...
			?></select>
			<br /><?php _e( 'OR', 'w4-post-list' ); ?><br /><a class="button" href="<?php echo esc_url( admin_url( 'post-new.php?post_type=' . W4PL_Config::LIST_POST_TYPE ) ); ?>"><?php _e( 'Create a new one', 'w4-post-list' ); ?></a>
		</p>
		<?php
	}
}
